    ...
    return (
      <div className='building-container'>
        ...
        <div className='building-item'>
          {!this.state.info ?
            <>
              <div className='building-item-title center-content'>{this.props.title}</div>
              <div className='building-item-icon center-content'>
          ...
          <div className='building-item-info'>
            <div className='button' onClick={()=>this.setState({info: !this.state.info, floorCount: this.props.floorCount(this.props.title,false)})}>i</div>
          </div>
        </div>
        <div onClick={this.purchaseFloor} className='building-purchase center-content'><AwesomeButton className='aws-button' type='primary'>{Koji.config.strings.purchase}</AwesomeButton></div>
      </div>
    )
  }
}
...
class Buildings extends Component {
  ...
  componentDidMount() {
    ...
    for (let i = 0; i < this.props.floors.length; i++){
      ...
      floors.push(
        <div key={i} className='building center-content'>
          <Building
